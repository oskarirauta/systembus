...
int ubus_create(void) {
	...
	if ( int ret = ubus_add_object(ctx, &cpu_object); ret != 0 ) {
		log::error << APP_NAME << ": failed to add system::cpu object to ubus" << std::endl;
		log::error << "error: " << ubus_strerror(ret) << std::endl;
		return ret;
	}

	if ( int ret = ubus_add_object(ctx, &security_object); ret != 0 ) {
		log::error << APP_NAME << ": failed to add system::security object to ubus" << std::endl;
		log::error << "error: " << ubus_strerror(ret) << std::endl;
		return ret;
	}

	if ( int ret = ubus_add_object(ctx, &info_object); ret != 0 ) {
		log::error << APP_NAME << ": failed to add system::info object to ubus" << std::endl;
		log::error << "error: " << ubus_strerror(ret) << std::endl;
		return ret;
	}
